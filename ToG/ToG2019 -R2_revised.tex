...
challenges: an evaluation function that \textcolor{blue}{can} separate the best
controllers from the rest, and a series of operators that \textcolor{blue}{can} 
explore different possibilities in the controller search space. Within
the context of the TORCS racing simulator, in this paper\textcolor{blue}{,} we introduce
a kind of fitnessless selection policy based on competition and called
\textit{Grand Prix Selection} (GPS), which will be able to increase robustness by using something more realistic than solo race scores to
...
reasonable speed \cite{Autodriv2006}.} Additionally, in a racing car\textcolor{blue}{,} the controller has to be designed so that the car wins in as many races as possible.
...
is chosen; fixed rules\textcolor{blue}{,} as well as neural networks \cite{KIM201287} or fuzzy controllers \cite{PerezEvolvingFuzzy09}\textcolor{blue}{,} are often used. \textcolor{blue}{
...
\textcolor{blue}{A potentially useful} way to \textcolor{blue}{increase robustness, that
...
eventually compete, in a race with other competitors. \textcolor{blue}{Hereafter, we will use in general the
...
\textcolor{blue}{
	If we can eliminate \textcolor{blue}{individually-computed} fitness and move to a ``fitnessless'' environment,, we can remove one source of uncertainty. We accomplish this by substituting a single (and uncertain) fitness  by a podium (a ranking after several races against other opponents) 
	...
	races several times."}
\textcolor{blue}{By doing this, we try to overcome one of the biggest
...
\textcolor{blue}{fuzzy-controller based car driving}
system. \textcolor{blue}{This system was iteratively improved in
...
\textcolor{blue}{to measure the performance} of the bot by doing solo
races, \textcolor{blue}{and we tried to make that surrogate as accurate as
...
far. \textcolor{blue}{Although this additional operator, by itself, did
...
are essential for evaluating a bot: \textcolor{blue}{it has many turns}, some of
them with 180 degrees, steep segments, and some very challenging parts
like the entrance of a tunnel at a square angle. \textcolor{blue}{ These features} make
...
To test the obtained controllers, we considered two tracks: the first track is \mbox{\textit{E-Track 5}}, used in previous papers \cite{salem_evo17},\cite{salem_evo18} \textcolor{blue}{and the second one is \mbox{\textit{Street 1}}.
...
in different ways.  \textcolor{blue}{This is why we have computed
  ...
  values after the first, the 30th, \textcolor{blue}{and} the last generation for the three controllers;} \textcolor{blue}{we take 30 measures of the
...
Figure \ref{fig:gfcsk} (top, middle, bottom) \textcolor{blue}{is the
...
\begin{figure}[ht]	
	\begin{center}
		...
		\caption{\textcolor{blue}{Excess skewness ($y$) and kurtosis ($x$)
...
procedure, and we consider it has been achieved\textcolor{blue}{, as it can be seen in
...
\textcolor{blue}{The main intention \textcolor{blue}{of} these experiments is to
...
\begin{table}[ht]
  ...
  {\scriptsize
    \caption{ Results of {\sf GFC-GPSVAE} and {\sf GFC-GPSE}
      ...
      best. \textcolor{blue}{We sort controllers according to the
...
\textcolor{blue}{Let it be noted that there are very few individual
...
\begin{figure}[h]	
  \begin{center}
    \includegraphics[width=8.5cm, height=1.7cm]{fig/FRONT.jpg}
    \includegraphics[width=8.5cm, height=1.7cm]{fig/MAX5.jpg}
    \includegraphics[width=8.5cm, height=1.7cm]{fig/MAX10.jpg}		
...
\end{table} 
\textcolor{blue}{{\sf GFC-GPSVAE} and {\sf GFC-GPSE} controllers were the best where
...
to try and find the best ones. \textcolor{blue}{Since the GPS method
...
\textcolor{blue}{Robustness in results has been our main objective, but
