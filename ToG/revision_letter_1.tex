...
%		 
		...
		% We understand that we could call it `Varying BLX-$\alpha$' instead of `adaptive BLX-$\alpha$' but the term `adaptive' refers to the fact that the algorithm adapts its need first for exploration and then for exploitation over generations. Since the need for exploration decreases as the GA advances towards the last generations, we have chosen a decreasing value of alpha.
                % Lots of words to illustrate the reviewer. You NEVER do that. The reviewer is always right.
...
We have added a text referring to our previous works in which we proved this statement obtaining competitive controllers just using these inputs.\\
			...
			\item	reduced diversity is good (1) -> usually, the opposite is true in my experience
					\item {\bf Response:} 
                                          We thank the reviewer for the interesting remark. In general, reduced diversity is not good. Reduced uncertainty is, because it means that the solutions are more robust. However, reduced undertainty might lead to a loss of diversity, since specific individuals are selected with a higher probability than in an uncertain context, where any individual has a non-zero possibility of being selected, as long as in that specific generation, the fitness measured is high enough. This is why in our paper the possible loss of diversity through selection is offset by the use of an operator, BLX-$\alpha$, with a certain degree of exploration through crossover and mutation. This has been clarified in the pagraph, which now reads: {\em This reduction of uncertainty has been proved also to reduce diversity in the genetic pool {\sf DBLP:journals/tcci/MereloLFGCCRMGTCC16}, which is why the introduction of the BLX-$\alpha$ operator, with a certain explorative component, achieves better results than GPS by itself, as we will show here.
...
% ------------------------------------------------------------------------
		...
		\item {\bf 	  Comment 6: While the paper is generally well-structured, there are several problems with the writing, sometimes making the message unclear. Examples are:}
			...
			\item "use neuroevolution instead of evolving" -> both are evolutionary approaches?
					\item {\bf Response:} 
					The intention was to write {\em simply evolving parameters}, but eventually we've decided to eliminate that sentence to make it clearer. Thanks for the suggestion.
...
The reviewer is right; although there's an implicit sequence (some initial exploration of the problem space, and eventual constraining to optimizing a few variables) we have changed that text, which now reads: {\em Since there are so many variables in this problem, the search space is usually reduced using heuristic rules for some of them}
				\item 	"in this environment is quite hard" -> very vague
 				\item {\bf Response:} % We agree with the reviewer, so we have rewritten the sentence in another way.
                                  % Again, not asking to write it in another way, which is just as vague. You need to make it less vague.
...
% NEVER EVER correct the reviewer if you don't want him or her to be pissed off. NEVER. I mean, really.
