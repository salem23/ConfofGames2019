...
We have added a text referring to our previous works in which we proved this statement obtaining competitive controllers just using these inputs.\\
			...
			Yes in general, diversification is good especially to avoid local minimums but when we are closely interested in the functioning of the genetic algorithm, we conclude that the need for diversification decreases along the iterations.
			A great diversification is necessary at the beginning to speed up the optimization process and get closer to the optimal solution but towards the end we need to press the selection in order to exploit the neighborhood of this solution.
			So more diversification in general implies a great deal of uncertainty in our case. What we are looking for is controlled diversification.
			\end{itemize}
...
% ------------------------------------------------------------------------
		...
		\item {\bf 	  Comment 6: While the paper is generally well-structured, there are several problems with the writing, sometimes making the message unclear. Examples are:}
			...
			\item "use neuroevolution instead of evolving" -> both are evolutionary approaches?
					\item {\bf Response:} *** TO CHECK ***\\
					Yes they are, but  neuroevolution  uses evolutionary algorithms to generate artificial neural networks by optimizing their parameters.
			\item What does "map the trajectory" mean?
					\item {\bf Response:} *** TO CHECK ***\\
			To find or estimate the position of the car in the track ( trajectory) using input data and evolutionary methods( Mapping inputs to find the trajectory).		
			...
			\item It is not clear whether the statistics about the articles cited are about TORCS or not? And if they aren't, why are they relevant?
					\item {\bf Response:} *** TO CHECK ***\\
			Yes, the mentioned statistics are about TORCS  environment.
			Statistics are given to position both the simulation environment and the used techniques in the research area. These statistics prove that researchers still working on them.
...
%24
% 45 
% Antonio - I guess these numbers shouldn't be here. ;D
% of course % Mohammed ------------------------------------------------------------------------
	...
	\item {\bf   Comment 7: Minor language comments: :} 
				...
				\item {\bf Response:} *** TO CHECK ***\\
				The TORCS environment has many uncontrolled parameters such as track conditions, tires condition and friction. These uncertain factors make it  difficult even hard to tune controller parameters via an optimization algorithm such as evolutionary ones.
