...
the diversity-increasing BLX-$alpha$ operator, with varying value of
$\alpha$, in combination with
...
\cite{DBLP:conf/cig/SalemMG19} we introduced the BLX-$alpha$ operator,
and checked two different configurations using a fixed and a
decreasing $\alpha$. The
results with a decreasing $\alpha$ were the best obtained so far, so
in this paper we are testing 
...
curves and straight parts, aiming to optimize the controllers for a more general behaviour, able to deal with different configurations in real circuits.
...
operator while the second uses  BLX with varying $\alpha$.
...
\begin{figure}[!ht]	
	\begin{center}
		...
		\caption{Skewness ($y$) and kurtosis ($x$) for a
                  ...
                  {\sf GFC-GPSVAE}, which is fitnessless and also uses
...
The structure of the winner controller $GFC-GPSVAE$ is depicted in Figures \ref{fig:frontmfs}, \ref{fig:max5mfs} and \ref{fig:max10mfs}
...
%% values of average running time values ( 123800s for instance)
			...
			\hline\textbf{$GFC-GPSVAE$}&2489	&11-18\\					
			\hline 
		\end{tabular}
