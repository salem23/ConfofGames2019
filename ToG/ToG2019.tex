...
\begin{figure}[!ht] 
	\begin{center}
		...
		\caption {TORCS capture showing some of the sensors
                  used by the fuzzy controllers. Figure from \cite{DBLP:conf/cig/SalemMG19}.}
		\label{fig:torcs-sensors}
	\end{center}
\end{figure}


As meny other studies in the area of autnomous driving or simulated
...
or angle; these
measurements are connected to car sensors \cite{manualTORCS}, which
are represented in Figure \ref{fig:torcs-sensors}; TORCS provides
these sensor values as input to the car controller, which is what we
are going to be evolving in this paper. Other open source simulators, mentioned in
\cite{Loiacono:2012:LEA:2212908.2212953}, are also available: VDrift
...
competitions, for which we could, eventually, submit our evolved bots.
...
\begin{figure}[!ht]
  ...
  \end{center}
  \caption{Schema of the two controllers that will be evolved by the
    evolutionary algorithm (GA), and evaluated by TORCS, which will
    return average and top speed, damages and lap times. Figure taken
    from \cite{salem_evo18}.}
\end{figure}
%
The two fuzzy subcontrollers we evolve take care of speed and steering
\cite{DBLP:conf/evoW/SalemMMG17}; these are shown in Figure
\ref{fig:ga}. The speed controller takes as input sensor values, and
outputs the target speed; the steering controller uses the same sensor
values to output steering orders.
What is evolved is the shape of the
fuzzy system, that is, the linguistic variables used as input. These
variables are encoded in 18 real-valued parameters. The low-level
details of this part has been explained extensively in
\cite{DBLP:conf/cig/SalemMG19}. As mentioned, these fuzzy controllers
proved to be a good platform for the evolution of racing car
controllers, and have not essentially changed since our first
paper. As a matter of fact, the methods presented in this paper could
...
In each process, three configurations are considered, for the first one , GPS is applied in every generation giving the new proposed controllers {\sf{GFC-GPSVAE}} and
...
{\sf{GFC-GPSVAL}}.
Also  and for comparison purpose, two controllers from previous papers are used, {\sf{GFC-VA}}\cite{DBLP:conf/cig/SalemMG19} and {\sf{GFC-VA}}\cite{salem_cig2018}, they both use the fitness function: $f_{AVS}$ (Equation \ref{fit2}).
...
configuration.  We kept the same evolutionary parameters as our previous controllers  to be able to compare them. % this should probably go to a table, and also needs
...
%last generation the second and each 5 generations in the last one. % Rewrite this and use acronyms as defined above - JJ
 ...
 % Clarify which ones have been used for the
...
\begin{table*}[ht]
	\centering
	{\scriptsize
		\caption{ Description of the controllers. The first and the second are propositions of this work}
		{
...
\end{table*}
%
To carry out the Grand Prix Selection with 5 races and 20 laps per race. In order to rank the participants, we considered score function based in Formula 1 Grand prix ranking, so the obtained punctuations depend on the car position in the final rank: 1 - 25 points, 2 - 18, 3 - 15, 4 - 12, 5 - 10, 6 - 8, 7 - 6, 8 - 4, 9 - 2, 10 - 1. The starting grid (initial positions of cars) on these races was set randomly.\\
...
three evolution processes {\sf GFC}, {\sf{GFC-GPSE}} and {\sf{GFC-VAGPS5}}. 
...
best between {\sf{GFC-GPSL}}, {\sf{GFC-GPSE}} and {\sf{GFC-GPS5}}; and also between {\sf{GFC-GPSVAL}}
,{\sf{GFC-GPSVAE}} and {\sf{GFC-GPSVA5}}.
...
\begin{table*}[ht]
	\centering
	{\scriptsize
		\caption{ Results of {\sf{GFC-GPS}} controllers in a mini-championship with 10 controllers and 10
...
\begin{table*}[ht]
	\centering
	{\scriptsize
		\caption{ Results of {\sf{GFC-VAGPS}} controllers in a mini-championship with 10 controllers and 10
			races in two different tracks(20 laps each). {\tt tita}, {\tt berniw} and {\tt
				inferno} are example controllers included with the TORCS
...
\end{table*}
%
As it could be noticed in the Table \ref{tab:RSresults}, the controller {\sf{GFC-VGPSAE}} has win the Grand Prix championship obtaining 98 points from 125 in the track used in the training, it has also ranked first in the track  \textit{E-Track 5}. //
The other GPS based controllers have obtained nearly the same results (149 and 158 respectively).
From Table \ref{tab:RSresults},it is clear that the controller {\sf{GFC-VAGPSE}}, where GPS selection has been applied in every generation, has won the majority of possible points in the two previous tracks even the unknown \textit{E-Track 5}  track.
...
processes: {\sf{GFC-VAGPSE}} and {\sf{GFC-GPSE}} are evaluated in formula like
...
The two {\sf{GFC-VAGPSE}} and {\sf{GFC-GPSE}} controllers were the best where
