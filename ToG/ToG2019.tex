...
As aforementioned the sub-controllers use three linguistic variables as inputs, derived from the sensors: one from Frontal, one from Sensors 8 and 10 ($\pm 5$\textdegree) and one from Sensors 7 and 11 ($\pm 10$\textdegree). These variables compose a Mamdani-based fuzzy system \cite{iancu2012} with three trapezoidal Membership Functions (MF) for every one.

Then, the controller is defined using a set of fuzzy rules which combine the fuzzy values for these inputs to get to an output: target speed or steering angle.

Thus, what will be evolved with the algorithm proposed in Section \ref{sec:ga} is the shape of the whole fuzzy system, which is encoded in 18 real-valued parameters (related to the different membership functions). The low-level details of this part has been explained extensively in \cite{DBLP:conf/cig/SalemMG19}. 

As mentioned, these fuzzy controllers proved to be a good platform for the evolution of racing car controllers, and have not essentially changed since our first paper. As a matter of fact, the methods presented in this paper could
