...
of this kind of selection via an adaptive BLX-$\alpha$ operator. We
...
Driving a \textcolor{red}{simulated} car is a problem in which you have to map inputs that include data about the environment we are driving in as well as car
...
laps''. This is what Vrajitoru does \cite{vrajitoru2019trajectory},
%\cite{vrajitoru2019trajectory,vrajitorugenetic}, 
%%% Antonio - I remove one reference to save space.
using then an evolutionary algorithm to optimize driving within that specific track. 
...
The second operator implemented in our GA is the Blend Crossover or  \textbf{BLX-$\alpha$ Crossover} \cite{blx2008}. \textcolor{red}{ stated before, one of the side effects of uncertainty is the higher diversification factor}. Thus, in order to have a better control between the exploration and exploitation factors during the evolution, we have used an \textit{adaptive approach} of the operator.
...
expected results are more reliable; this was an objective of this GPS procedure, and we consider it has been achieved. Although we have represented a single run here, results for other runs are similar.
