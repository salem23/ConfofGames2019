...
include data about the environment we're driving in as well as car
...
reasonable speed. Additionally, in a racing car the controller has to
be designed so that the car wins in as many races as possible.

There are so many variables in this problem, that eventually some of
them are chosen using heuristic rules, while the rest are open to
optimization. For instance, deciding the type of controller is usually
done before the desing process begin, and a single, parametrized one
is chosen; fixed rules as well as neural networks \cite{KIM201287} or fuzzy controllers
are often used; additional to that, training (or learning) \cite{Loiacono:2012:LEA:2212908.2212953} can be done during the
race, so that it can adapt itself to new, and not seen before,
scenarios and competitors, or it can be fixed by training the
controllers in advance. We have been using genetically-optimized fuzzy
controllers in this line of work, with increasing success.

In a simulated and realistic racing car environment like the one we use, however, racing car scores are a
    ...
    the result of scores in unknow tracks with unknown competitors
    uncertain at best.
...
eventually work, in a race with other competitors. By totally
eliminating fitness, in a fitness-less environment, we also eliminate
...
In this paper, then, we will check how this Grand Prix selection works
compared with mixed fitness-less and fitness races, or simply using a
selection for those controllers that have the highest fitness in the
last generation. This reduction of uncertainty has been proved also to
reduce diversity in the genetic pool
\cite{DBLP:journals/tcci/MereloLFGCCRMGTCC16}. In our previous paper,
  \cite{DBLP:conf/cig/SalemMG19}, we introduced the BLX-$\alpha$
  operator which works, at the same time, as exploratory and exploits
  the existing space of solutions. For this kind of Grand Prix
  selection, however, it will probably be much more convenient for the
  last phase of the races to just exploit existing solutions by
  combining them, so that we introduce an adaptive version of
  BLX-$\alpha$ that behaves in that way.

These two new techniques, Grand Prix selection (GPS) and a decreasing
$\alpha$ BLX-$\alpha$ operator will be cross compared in different
combinations, with the experiments proving that the combination of
both yields the best controllers we have been able to evolve so far. 
...
\ref{sec:methods}. After this, the evolutionary algorithm implemented is explained in Section \ref{sec:ga}, including the new selection method and
the new version of the BLX-$\alpha$ crossover operator. After it, the
...
\hline
      Value Front & Value MAX5 & Value MAX10 & Target Speed (km/h) \\ % what
                                % are units for target speed? - JJ
										  % Antonio - if I'm not wrong km/h. Is this 						
										  % Mohammed?
      \hline
    	 High & - & - & 280 \\
      Medium & - & - & 240 \\
...
% We need to specify what turning values mean.
% Antonio - I don't exactly remember, but I guess they are 0 -> no turn, 1 -> 90 degrees, however I think there is also a negative sign (to turn on the left).
% Mohammed, please clarify this and maybe write the angles instead of these values (as we did in the speed).
    \begin{tabular}{|c|c|c||c|}
\hline
...
{\sf{GFC-GPSVAE}}&Fitnessless&Every generations&BLX$-\alpha$\\

{\sf{GFC-GPS5}}\cite{DBLP:conf/cig/SalemMG19}&Mixed fitness&5 generations&Standard crossover\\
{\sf{GFC-GPSVA5}}\cite{DBLP:conf/cig/SalemMG19}&Mixed fitness&5 generations&BLX$-\alpha$\\

{\sf{GFC-GPSL}}\cite{DBLP:conf/cig/SalemMG19}&Mixed fitness&Last generation &Standard crossover\\
	
{\sf{GFC-GPSVAL}}\cite{DBLP:conf/cig/SalemMG19}&Mixed fitness&Last generation&BLX$-\alpha$\\
...
The structure of the winner controller $GFC-VAGPSE$ is depicted in Figures \ref{fig:frontmfs}, \ref{fig:max5mfs} and \ref{fig:max10mfs}
...
This paper is dedicated to present a new selection policy to replace the use of standard fitness in genetic algorithms to optimize a fuzzy controller for racing cars in the TORCS simulator. The performance of this type of algorithm fully depends on the choice of the appropriate fitness function and on certain operators’ parameters such as crossover and mutation. Furthermore, in car racing, we have an additional difficulty which is the uncertainty caused by opponents decisions and track state where we would be forced to handle non-deterministic fitness values.
...
The main idea of this work is to overcome uncertainty by using a selection policy based on a system similar to Grand Prix championships where the score of a controller is the sum of the points collected during a set of races and in order to speed up the optimization process, we have introduced a new crossover operator: BLX-$\alpha$ which improves exploration.
A statistical study has led us to affirm that the proposal makes it possible to face uncertainty thus giving a robust and reliable controller and the introduction of the operator BLX-$\alpha$ has allowed a clear acceleration of the process of genetic optimization while the comparison of the optimized controller with other ones from the literature has shown the robustness of the fuzzy controller where it has won the majority of races being the first in the general ranking. 
