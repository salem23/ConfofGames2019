...
The problem of designign controllers for racing cars has been
...
However, it's not the only kind of soft computing method
used. \cite{mirus2019short} uses neuromorphic architecture, namely,
spiking neural networks, but using, the same way we do, a single track
for training. The kind of data used for the design of racing cars is
also different depending on the author. We think that, in general, successful racing will only
need these sensor values to drive the car, since they give enough
information to avoid collisions, drive as close as possible to the
center of the track, and drive as fast as possible when needed.

Soft computing has been combined also with evolutionary algorithms in
several occasions: \cite{10.1371/journal.pone.0213193} uses
...
called ``transfer learning''. This is the technique used by
\cite{verma2018programmatically}. 


Evolution can also be used directly to map the trajectory after ``test
laps''. This is what \cite{vrajitoru2019trajectory,vrajitorugenetic}
does, using then an evolutionary algorithm to optimize driving within
that specific track. 
...
use vision, using TORCS as a testbed for a different kind of
algorithms, on the other hand \cite{8833873}
uses deep learning, working with real images instead of sensor values,
the way we do, \cite{Kaushik_2018_ECCV_Workshops} describes a DDPG (deep
...
The concept, however, was goes way back at leat to
\cite{Angeline:1993:CEE:645513.657590}, which calles it a "competitive
fitness environment". Fitnessless evolution was further developed in
\cite{rosin1995methods}, but it's been also used in genetic
programming by Tettamanzi in \cite{tettamanzi1996genetic}.

However, the competitions played in those works were not, strictly,
games. The first time fitness-less evolution was used in  games was by
\cite{luke1998genetic}; however, it was later used extensively, like
\cite{Jaśkowski2008,10.1007/978-3-540-78671-9_2,alhejali2011using}. Deriqving
which individuals are the best from these races, taking into account
