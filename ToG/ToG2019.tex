...
\textit{Grand Prix Selection} (GPS), which \textcolor{red}{will} be able to reduce uncertainty by using something more realistic than solo race scores to
select individuals. Additionally, we increase the exploitative power
of this kind of selection via a BLX operator  \textcolor{red}{with continuously decreasing $alpha$}. We
...
operators. In general, \textcolor{red}{experiments} show that these combined
...
Driving a \textcolor{red}{simulated} car \textcolor{red}{can be formulated as an optimization} problem in which you have to map inputs that include data about the environment we are driving in as well as car
data, to the output: throttling and steering \textcolor{red}{to meet a series of standards}: cars should not crash and should have a
...
\textcolor{red}{This reduction of uncertainty has been proved also to reduce diversity in the genetic pool \cite{DBLP:journals/tcci/MereloLFGCCRMGTCC16}, which is why the introduction of the BLX-$\alpha$ operator, with a certain explorative component, achieves better results than GPS by itself, as we will show here.
In addition, we have compared the influence of the BLX-$\alpha$ operator on the performance of the evolved controllers with others using standard crossover operators, since we aim to better control the diversity in the population.}
...
The second operator implemented in our GA is the Blend Crossover or \textbf{BLX-$\alpha$ Crossover} \cite{blx2008} \textcolor{red}{introduced originally by Eshelman and Schaeffer \cite{eshelman1993real}, and adapted here to use a varying $\alpha$ value}. \textcolor{red}{As stated before, one of the side effects of uncertainty is its derived higher diversification factor}. Thus, in order to have a better control between the exploration and exploitation factors during the evolution, \textcolor{red}{the balance changes with the decreasing value of $\alpha$}.
...
In the second and third configuration, we retrained our previous four  controllers \cite{DBLP:conf/cig/SalemMG19}: {\sf{GFC-GPS5}} and 
...
\textcolor{red}{Although we have represented a single run here, results for other runs are similar.}
...
\begin{table}[h!]
	\centering
	{\scriptsize
		\caption{ \textcolor{red}{Results of GPS controllers  and varying $BLX-\alpha$ controllers in a mini-championship
...
\textcolor{red}{This final competition shows {\sf GFC-GPSVAE} clearly leading. It collected most of the points in the four tracks, leaving the {\sf GFC-GPSE} controller far behind it. The reference drivers, namely {\sf GFC-VA} and {\sf GFC}  are classified $3^{rd}$ and $4^{th}$.
}
...
has as many different features as possible. % Added this, check it out
                                % - JJ
