...
accelerator, steering wheel or gearbox. The controller output is not
evolved, and is shown in Tables \ref{tab:output:speed} and \ref{tab:ouput:steer}. This has not been
varied along this line of research, and has been designed following
usual race car driving rules. % Please check this - JJ
%
\begin{table*}[h!tb]
  \centering
  {\scriptsize
...
\hline
      Value Front & Value M5 & Value M10 & Target Speed \\
\hline
\hline
\end{tabular}
}
...
\begin{table*}[h!tb]
  \centering
  {\scriptsize
    \caption{Rules for steering controller output.\label{tab:output:steer}}
    \begin{tabular}{|c|c|c||c|}
\hline
      Value Front & Value M5 & Value M10 & Steer \\
\hline
\hline
\end{tabular}
}
...
We have been using an evolutionary algorithm (EA) \cite{GAs_Goldberg89,EAs_Back96} to
optimize the parameters of the membership functions of the fuzzy
sub-controllers we have been using in this line of research. EAs are
nature inspired methods that evolve populations of possible (encoded)
solutions for a problem following a process of selection of the best,
...
applied to solve regression problems and fuzzy systems
\cite{hoffmann2001evolutionary}; in this kind of problem the
solutions are modelled as a vector of numeric values, as is the
case in this paper. We show Figure \ref{fig:cromosome} how we
represent every individual (chromosome) as a vector of 18
values/parameters (genes), 6 per variable. They have been coded using
real values, looking for higher precision \cite{elsayed13}. We started
to use this encoding in \cite{10.1007/978-3-319-77538-8_24}.
