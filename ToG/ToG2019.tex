...
This section presents the environment where the study has been conducted, i.e. the simulator named TORCS, as well as the fuzzy subcontrollers designed in previous works and which will be the `target' of the optimization process conducted in this paper.
...
%----------------------------  FUZZY SUBCONTROLLERS ----------------------------

\subsection{Fuzzy Subcontrollers}
...
\begin{figure}[!ht]
  ...
  \caption{Schema of the two controllers that will be evolved by the
    ...
    return the values for the car's average speed on the whole race and the incurred damage on the vehicle. Figure taken from \cite{salem_evo18}.}
\end{figure}
%
...
The speed controller takes as input sensor values, and
...
\section{Optimizing Subcontrollers with a Genetic Algorithm}
\label{sec:ga}
