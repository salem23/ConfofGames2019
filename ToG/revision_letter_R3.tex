...
- 2) When learning in a competitive environment, does the learned controller work well in a solo environment? Additional experiments in this regard appear to be necessary.
			\end{quote}	
		\item {\bf Response:} 
		We thank the reviewer for the comment, We agree with the reviewer that additional experiments are needed to test the learned controller in solo environment but the aim of our paper is to design a controller for TORCS races trying to eliminate uncertainty and we argue that competitive environment are more uncertain and difficult to train a good bot.
