...
%%\textcolor{red}{
		\begin{itemize}
			...
			\item {\bf Response:} 
				%%Mohammed 
					We thank the reviewer for the comments, they are all revised and highlighted in blue color in the text.
			\item {\bf Comment 2:}
				\begin{quote}
...
\item {\bf \underline{ Reviewer 2 Comments}}\\
	...
	\begin{itemize}			
		...
		\item {\bf Response:} 
			%%Mohammed 
		We have rewritten this part of the text to avoid confusion. You can find it highlighted in blue.
...
%		We believe that the uncertainty in race competition is due to the lack of the information about the driving environment or 
		...
		\item {\bf Response:}
			%%Mohammed
				We agree with the reviewer's comment. the sentence is a bit confusing. It has been rewritten following the reviewer's suggestion and marked in blue in the text. So, now it is:
...
the BLX-$alpha$ operator and the ``Grand Prix Selection''.
		...
		\item {\bf Comment 7:}
			...
			\end{quote}	
		\item {\bf Response:} 
				Following the reviewer's suggestion we have rewritten the sentence. We have marked the new paragraph in blue in the text.
				%%Antonio - DONE
		\item {\bf Comment 8:}
			\begin{quote}
				Section 5:
		...
		\textcolor{blue}{	{\bf "GFC-GPSVAE and GFC-GPSE controllers were the best where 	they are ranked $1^{st}$ and $2^{nd}$ in the comparative competition(See Table.4) but they are very expensive in runtime."}}	
		\item {\bf Comment 10:}
			\begin{quote}
...
the intent of our method, and also the intent of our original response.
			...
			\textcolor{blue}{This is why we have computed skewness
...
%				%% Mohammed
			...
			\item{\bf Response:} 
			 ...
			 \textcolor{blue}{	{\bf in general, successful racing could be achieved using only these sensor values} }
			\item {\bf Comment 5:}
				\begin{quote}	
